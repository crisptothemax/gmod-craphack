...
public:
	...
	void					InitArray(int nElements, int elementStride) noexcept;

	int						GetNumElements() const noexcept;
	void					SetNumElements(int nElements) noexcept;

	int						GetElementStride() const noexcept;
	void					SetElementStride(int stride) noexcept;

	int						GetFlags() const noexcept;

	const char* GetName() const noexcept;
	SendPropType			GetType() const noexcept;

	RecvTable* GetDataTable() const noexcept;
	void					SetDataTable(RecvTable* pTable) noexcept;

	RecvVarProxyFn			GetProxyFn() const noexcept;
	void					SetProxyFn(RecvVarProxyFn fn) noexcept;

	DataTableRecvVarProxyFn	GetDataTableProxyFn() const noexcept;
	void					SetDataTableProxyFn(DataTableRecvVarProxyFn fn) noexcept;

	int						GetOffset() const noexcept;
	void					SetOffset(int o) noexcept;

	// Arrays only.
	RecvProp* GetArrayProp() const noexcept;
	void					SetArrayProp(RecvProp* pProp) noexcept;

	// Arrays only.
	void					SetArrayLengthProxy(ArrayLengthRecvProxyFn proxy) noexcept;
	ArrayLengthRecvProxyFn	GetArrayLengthProxy() const noexcept;

	bool					IsInsideArray() const noexcept;
	void					SetInsideArray() noexcept;

	// Some property types bind more data to the prop in here.
	const void* GetExtraData() const  noexcept;
	void				SetExtraData(const void* pData) noexcept;

	// If it's one of the numbered "000", "001", etc properties in an array, then
	// these can be used to get its array property name for debugging.
	const char* GetParentArrayPropName() noexcept;
	void				SetParentArrayPropName(const char* pArrayPropName) noexcept;
...
public:
	...
	RecvTable();
	RecvTable(RecvProp* pProps, int nProps, const char* pNetTableName) noexcept;
	~RecvTable();

	void		Construct(RecvProp* pProps, int nProps, const char* pNetTableName) noexcept;

	int			GetNumProps() noexcept;
	RecvProp* GetProp(int i) noexcept;

	const char* GetName()  noexcept;

	// Used by the engine while initializing array props.
	void		SetInitialized(bool bInitialized) noexcept;
	bool		IsInitialized() const noexcept;

	// Used by the engine.
	void		SetInMainList(bool bInList) noexcept;
	bool		IsInMainList() const noexcept;
...
inline int RecvTable::GetNumProps() noexcept
{
	return m_nProps;
}

inline RecvProp* RecvTable::GetProp(int i) noexcept
{
	// Assert( i >= 0 && i < m_nProps ); 
	return &m_pProps[i];
}

inline const char* RecvTable::GetName() noexcept
{
	return m_pNetTableName;
}

inline void RecvTable::SetInitialized(bool bInitialized) noexcept
{
	m_bInitialized = bInitialized;
}

inline bool RecvTable::IsInitialized() const noexcept
{
	return m_bInitialized;
}

inline void RecvTable::SetInMainList(bool bInList) noexcept
{
	m_bInMainList = bInList;
}

inline bool RecvTable::IsInMainList() const noexcept
{
...
void RecvProxy_FloatToFloat(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;
void RecvProxy_VectorToVector(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;
void RecvProxy_QuaternionToQuaternion(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;
void RecvProxy_Int32ToInt8(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;
void RecvProxy_Int32ToInt16(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;
void RecvProxy_StringToString(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;
void RecvProxy_Int32ToInt32(const CRecvProxyData* pData, void* pStruct, void* pOut) noexcept;

// StaticDataTable does *pOut = pData.
void DataTableRecvProxy_StaticDataTable(const RecvProp* pProp, void** pOut, void* pData, int objectID) noexcept;

// PointerDataTable does *pOut = *((void**)pData)   (ie: pData is a pointer to the object to decode into).
void DataTableRecvProxy_PointerDataTable(const RecvProp* pProp, void** pOut, void* pData, int objectID) noexcept;
...
inline void RecvProp::InitArray(int nElements, int elementStride) noexcept
{
...
inline int RecvProp::GetNumElements() const noexcept
{
	return m_nElements;
}

inline void RecvProp::SetNumElements(int nElements) noexcept
{
	m_nElements = nElements;
}

inline int RecvProp::GetElementStride() const noexcept
{
	return m_ElementStride;
}

inline void RecvProp::SetElementStride(int stride) noexcept
{
	m_ElementStride = stride;
}

inline int RecvProp::GetFlags() const noexcept
{
	return m_Flags;
}

inline const char* RecvProp::GetName() const noexcept
{
	return m_pVarName;
}

inline SendPropType RecvProp::GetType() const noexcept
{
	return m_RecvType;
}

inline RecvTable* RecvProp::GetDataTable() const noexcept
{
	return m_pDataTable;
}

inline void RecvProp::SetDataTable(RecvTable* pTable) noexcept
{
	m_pDataTable = pTable;
}

inline RecvVarProxyFn RecvProp::GetProxyFn() const noexcept
{
	return m_ProxyFn;
}

inline void RecvProp::SetProxyFn(RecvVarProxyFn fn) noexcept
{
	m_ProxyFn = fn;
}

inline DataTableRecvVarProxyFn RecvProp::GetDataTableProxyFn() const noexcept
{
	return m_DataTableProxyFn;
}

inline void RecvProp::SetDataTableProxyFn(DataTableRecvVarProxyFn fn) noexcept
{
	m_DataTableProxyFn = fn;
}

inline int RecvProp::GetOffset() const noexcept
{
	return m_Offset;
}

inline void RecvProp::SetOffset(int o) noexcept
{
	m_Offset = o;
}

inline RecvProp* RecvProp::GetArrayProp() const noexcept
{
	return m_pArrayProp;
}

inline void RecvProp::SetArrayProp(RecvProp* pProp) noexcept
{
	m_pArrayProp = pProp;
}

inline void RecvProp::SetArrayLengthProxy(ArrayLengthRecvProxyFn proxy) noexcept
{
	m_ArrayLengthProxy = proxy;
}

inline ArrayLengthRecvProxyFn RecvProp::GetArrayLengthProxy() const noexcept
{
	return m_ArrayLengthProxy;
}

inline bool RecvProp::IsInsideArray() const noexcept
{
	return m_bInsideArray;
}

inline void RecvProp::SetInsideArray() noexcept
{
	m_bInsideArray = true;
}

inline const void* RecvProp::GetExtraData() const noexcept
{
	return m_pExtraData;
}

inline void RecvProp::SetExtraData(const void* pData) noexcept
{
	m_pExtraData = pData;
}

inline const char* RecvProp::GetParentArrayPropName() noexcept
{
	return m_pParentArrayPropName;
}

inline void	RecvProp::SetParentArrayPropName(const char* pArrayPropName) noexcept
{
